...
class RealtimeDialogClient:
    ...
    async def connect(self) -> None:
        ...
        self.ws = await websockets.connect(self.config["base_url"], extra_headers=self.config["headers"], ping_interval=None)
    ...
    async def say_hello(self) -> None:
        payload = {"content": "你好，我是豆包，有什么可以帮助你的？"}
    ...
    async def task_request(self, audio: bytes) -> None:
        task_request = bytearray(protocol.generate_header(message_type=protocol.CLIENT_AUDIO_ONLY_REQUEST, serial_method=protocol.NO_SERIALIZATION))
